...
async function waitForUploadResult(sessionId: string, requestId: string) {
  ...
  while (Date.now() - startedAt < POLL_TIMEOUT_MS) {
    ...
    if (!result.success) throw new Error(result.message || '读取原图上传结果失败');
    if (result.status === 'completed') return result.files || [];
    if (result.status === 'cancelled') return [];
    if (result.status === 'failed') throw new Error(result.message || '原图上传失败');
    await sleep(POLL_INTERVAL_MS);
  }
  throw new Error('等待小程序上传超时，请重新选择图片');
}

async function chooseWithNativeBridge(input: HTMLInputElement) {
  ...
  return waitForUploadResult(sessionId, created.requestId);
}

function dispatchNativeFiles(input: HTMLInputElement, uploaded: NativeUploadFile[]) {
  ...
  uploaded.forEach((item, index) => {
    ...
    registerNativeUploadResult(file, { fileID: item.fileID });
...
export function installNativeImageUploadBridge() {
  if (!isMiniProgramWebView() || typeof DataTransfer === 'undefined') return () => {};
  ...
  const handleClick = async (event: MouseEvent) => {
    ...
    try {
      const uploaded = await chooseWithNativeBridge(input);
      if (uploaded.length) dispatchNativeFiles(input, uploaded);
    } catch (error) {
