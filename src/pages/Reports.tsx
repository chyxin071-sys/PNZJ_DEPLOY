...
export default function ReportsPage() {
  ...
  return (
    <div className="erp-page-spaced">
      ...
      <div className="rounded border border-amber-100 bg-amber-50/60 px-4 py-3 text-xs leading-5 text-amber-700">
...
function TargetProgressCard({ title, value, target }: { title: string; value: number; target: number }) {
  ...
  return (
    <div className={`rounded border bg-white p-4 ${reached ? 'border-emerald-100' : 'border-gray-100'}`}>
...
function ChartCard({ title, children, className = '' }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`rounded border border-gray-100 bg-white p-3 md:p-5 ${className}`}>
          ...
          return (
            <div key={`mobile-${d.month}`} className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => onDrill(d.monthNo, 'income')} className="rounded bg-emerald-50 px-3 py-2 text-left">
                ...
                <button type="button" onClick={() => onDrill(d.monthNo, 'expense')} className="rounded bg-red-50 px-3 py-2 text-left">
                ...
                {mode !== 'cash' && (
                  <div className="col-span-2 flex items-center justify-between rounded bg-gray-50 px-3 py-2 text-xs">
