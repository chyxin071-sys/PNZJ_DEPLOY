  ...
  return (
    <div className="erp-page-spaced">
      <header className="erp-page-header">
        ...
        <button type="button" onClick={() => void refresh()} title="刷新设备" className="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50"><RefreshCw size={17} className={loading ? 'animate-spin' : ''} /></button>
      </header>

      <section className="grid gap-4 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <div className="erp-surface p-5 md:p-6">
          <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gold-50 text-gold-600"><ShieldCheck size={21} /></div><div><h2 className="text-base font-semibold text-gray-900">授权新大屏</h2><p className="mt-1 text-xs leading-5 text-gray-500">在大屏打开 pinnuozhujia.cn/erp/operations-screen，再输入屏幕上的 6 位授权码。</p></div></div>
          <div className="mt-6">
            <label className="text-xs font-medium text-gray-600">设备名称</label>
            <input value={deviceName} onChange={(event) => setDeviceName(event.target.value)} maxLength={30} className="mt-2 h-11 w-full rounded-lg border border-gray-200 px-3 text-sm outline-none focus:border-gold-400 focus:ring-2 focus:ring-gold-100" />
          </div>
          <div className="mt-4">
            <label className="text-xs font-medium text-gray-600">6 位授权码</label>
            <input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" placeholder="请输入大屏上的授权码" className="mt-2 h-14 w-full rounded-lg border border-gray-200 px-4 text-center text-2xl font-bold tracking-[0.35em] outline-none focus:border-gold-400 focus:ring-2 focus:ring-gold-100" />
          </div>
          <button type="button" onClick={() => void handleApprove()} disabled={submitting || code.length !== 6} className="mt-5 flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-gray-900 text-sm font-medium text-white disabled:cursor-not-allowed disabled:bg-gray-300"><MonitorSmartphone size={17} />{submitting ? '正在授权...' : '授权此大屏'}</button>
