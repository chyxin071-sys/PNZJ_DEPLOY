  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-3 py-3 md:px-6 md:py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 shrink-0 sticky top-0 z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3">
          <div className="grid grid-cols-2 md:flex md:items-center gap-2 md:mr-4">
            ...
            {isViewMode ? (
              <span className="h-8 inline-flex items-center rounded-lg border border-gray-200 px-2 text-xs md:text-sm font-medium text-gray-900 md:mr-2">{version}</span>
            ) : (
              <input value={version} onChange={e => setVersion(e.target.value)} className="h-8 w-full md:w-20 px-2 py-1 text-xs md:text-sm border rounded-lg" />
            )}
            {isViewMode ? (
              <span className="h-8 inline-flex items-center justify-center px-2 py-1 text-xs rounded-lg bg-gray-100">{status}</span>
            ) : (
              <select value={status} onChange={e => setStatus(e.target.value as any)} className="h-8 w-full md:w-auto px-2 py-1 text-xs md:text-sm border rounded-lg bg-white">
          ...
          <div className="text-xs md:text-sm font-bold text-gray-900 bg-gray-100 px-3 py-2 md:py-1.5 rounded-lg flex items-center justify-between md:justify-start gap-2">
          ...
          <button onClick={handleExport} className="h-9 px-3 md:px-4 text-xs md:text-sm bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 font-medium flex items-center justify-center gap-1.5 md:gap-2">
            <Download size={15} /> 导出
          </button>
          {isViewMode ? (
            <button onClick={() => navigate(`/quotation-builder/${sourceType}/${sourceId}/${quotationId}`)} className="h-9 px-3 md:px-4 text-xs md:text-sm bg-gray-900 text-white rounded-lg hover:bg-gray-800 font-medium flex items-center justify-center gap-1.5 md:gap-2">
              <Edit3 size={15} /> 编辑
            </button>
          ) : (
            <button onClick={handleSave} disabled={saving} className="h-9 px-3 md:px-4 text-xs md:text-sm bg-gray-900 text-white rounded-lg hover:bg-gray-800 font-medium flex items-center justify-center gap-1.5 md:gap-2">
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-3 md:p-6">
        <div className="flex gap-1 bg-white p-1 rounded-lg shadow-sm border border-gray-100 shrink-0 mb-3 md:mb-4 overflow-x-auto scrollbar-hide">
