...
export default function QuotesBiz() {
  ...
  return (
    <div className="erp-page space-y-5">
      ...
      <div className="bg-white rounded shadow-sm border border-gray-100 overflow-visible flex flex-col">
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 justify-between">
          <div className="relative w-full sm:w-80">
            ...
            <input type="text" placeholder="搜索客户名称、编号..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-gray-900 transition-colors" />
          </div>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            className="px-3 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-gray-900 transition-colors bg-white">
              ...
              {filtered.map(q => (
                <tr key={q.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button onClick={() => handleView(q)} className="p-1.5 text-gray-400 hover:text-gray-900 rounded hover:bg-gray-100 transition-colors" title="查看">
                      <Eye size={16} />
                    </button>
                    <button onClick={() => handleDelete(q.id)} className="p-1.5 text-gray-400 hover:text-red-500 rounded hover:bg-red-50 transition-colors" title="删除">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
