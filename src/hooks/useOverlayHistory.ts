...
export function useOverlayHistory(open: boolean, onClose: () => void, stateKey = 'pnzjOverlayId') {
  ...
  useEffect(() => {
    ...
    const handlePopState = (event: PopStateEvent) => {
      ...
      // A nested overlay may have been popped back to this overlay's entry.
      // In that case this overlay is still the active top layer and must stay open.
      if (event.state?.[OVERLAY_TOP_KEY] === overlayId) return;
